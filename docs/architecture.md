...
- Implemented pack families now include `provider_governance`, `fundamental_rights`, `post_market_monitoring`, and `conformity` alongside the Annex/runtime/risk/GPAI slices.
...
- That `pl-merkle-sha256-v4` JSON-pointer redaction support is the compatibility mechanism that lets the schema grow richer nested evidence fields without redesigning selective disclosure each time a governance item gains more structure.
...
- Pack curation can now also key off `compliance_profile` fields such as `fria_required=true`, so deployer-side FRIA exports can be assembled from the same bundle inventory as general capture flows.
...
- `proofctl vault restore` now provides the matching offline import path for that archive format, restoring into a fresh local directory instead of mutating a live service in place. When backup encryption is enabled, restore decrypts the envelope with `--backup-key` or `PROOF_SERVICE_BACKUP_ENCRYPTION_KEY_B64` before unpacking.
- `proof-service` can also run scheduled local backup export when `[backup].interval_hours` / `PROOF_SERVICE_BACKUP_INTERVAL_HOURS` is greater than zero, writing archives into `[backup].directory` / `PROOF_SERVICE_BACKUP_DIR`, pruning old `proof-layer-vault-backup-*.tar.gz` or `.tar.gz.enc` files down to the configured retention count, and excluding that backup directory from the archived `storage/*` tree to avoid recursive growth.
- Backup export can now also be wrapped in an XChaCha20-Poly1305 envelope when `[backup.encryption]` or `PROOF_SERVICE_BACKUP_ENCRYPTION_KEY_*` is configured. The service exposes only non-secret encryption metadata (`enabled`, algorithm, key id) through `GET /v1/config`.
...
- `GET /v1/config` returns the active service view for payload limits, bound address, TLS enabled state, auth enabled state/principal labels, tenant enforcement state, signing algorithm/key id, storage backends, retention grace period, retention policies, scheduled-backup settings, and persisted timestamp/transparency provider settings.
...
- The package now also exposes first-class typed helpers for the full implemented evidence catalog, including interaction/runtime items, governance items (`instructions_for_use`, `qms_record`, `fundamental_rights_assessment`, `standards_alignment`, `post_market_monitoring`, `corrective_action`), authority-reporting items (`authority_notification`, `authority_submission`, `reporting_deadline`, `regulator_correspondence`), GPAI/provider items (`model_evaluation`, `adversarial_test`, `training_provenance`, `compute_metrics`, `downstream_documentation`, `copyright_policy`, `training_summary`), literacy/incident items, and the conformity/declaration/registration group.
- The `ProofLayer` facade can now apply a default `compliance_profile` across captures so role, intended-use, FRIA, and GPAI context do not have to be re-entered per item.
...
- The Python package now also exposes a higher-level `ProofLayer` facade, shared `evidence.py` request builders for the same full evidence catalog, and provider-specific `with_proof_layer(...)` wrappers for OpenAI-like and Anthropic-like clients, including governance artefacts, GPAI evaluation/provenance/documentation items, Art 4 literacy attestations, incident-response evidence, and the conformity/declaration/registration group.
- The Python facade also accepts a default `compliance_profile`, matching the TypeScript surface for SDK-first classification reuse.
