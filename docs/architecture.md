...
- The Python package now also exposes a higher-level `ProofLayer` facade, shared `evidence.py` request builders for all evidence item types currently implemented in Rust core, and provider-specific `with_proof_layer(...)` wrappers for OpenAI-like and Anthropic-like clients, including Art 4 literacy attestations and incident-report lifecycle evidence.
