...
- The checked-in fixture now uses `bundle_root_algorithm = "pl-merkle-sha256-v2"`, so `proof_bundle.canonical.json` contains the reduced signed header projection (`bundle_version`, `bundle_id`, `created_at`, `actor`, `subject`, `context`, `policy`, `item_count`, `artefact_count`) rather than the full bundle body.
- The fixture Merkle root is computed from `[header_digest, item_digest..., artefact_meta_digest...]`; artefact byte digests are still verified separately against the recorded artefact entries.
